                      ...
                      <td>
                        @if (Auth::user()->role == 'admin')
                          @if($perina->bedstatus==3)
                            <a href="{{ url('perinatologi/'.$perina->id.'/cleaned') }}" type="button" class="btn btn-block btn-sm btn-info" data-toggle='tooltip' data-placement='left' title='Update data {{$perina->namapasien}}'><i class="fas fa-bed"></i> <b>SELESAI CLEANING</b></a>
                          @else
                            -
                          @endif
                        @endif
                    ...
                    <tr>
                      <td>
                        ...
                        @else
                          -
                        @endif
                        <br>Waktu  Habis Cairan Infus: <b>@if ($perina->tci_waktuhabisinfus==null)
                            -
                        @else
                            {{\Carbon\Carbon::parse($perina->tci_waktuhabisinfus)->format('H:i:s | d-m-Y') }}
                        @endif</b>
                         <br> 
                ...
                <div class="card-footer">
                  <div class="btn-group container-fluid  p-0">
                    @if ($perina->bedstatus==7)
                      <a href="{{ url('perinatologi/'.$perina->trx_id.'/edit') }}" type="button" class="btn btn-sm btn-info" data-toggle='tooltip' data-placement='left' title='Update data {{$perina->namapasien}}'><i class="fas fa-bed"></i> <b>UPDATE</b></a>
                      ...
                      <a href="{{ route('perinatologi.rencanaPulang', $perina->trxPasien->trx_id) }}" type="button" class="btn btn-sm btn-danger" data-toggle='tooltip' data-placement='left' title='Rencanakan {{$perina->namapasien}} pulang' onclick="return  confirm('Apakah anda yakin?')"><i class="fas fa-user-injured"></i> <b>RENCANA PULANG</b></a>
                      
                      @elseif ($perina->bedstatus==8)
                      <a href="{{ url('perinatologi/'.$perina->trx_id.'/edit') }}" type="button" class="btn btn-sm btn-info" data-toggle='tooltip' data-placement='left' title='Update data {{$perina->namapasien}}'><i class="fas fa-bed"></i> <b>UPDATE</b></a>
                      <button class="btn btn-sm btn-warning" data-toggle="modal" data-target="#Modal-{{$perina->trx_id}}" data-toggle='tooltip' data-placement='left' title='Pindahkan {{$perina->namapasien}} ke bed atau ruangan lain'><i class="fas fa-bed"></i> <b>MUTASI</b></button>
                      <a href="{{ url('perinatologi/'.$perina->trx_id.'/rencanapulang') }}" type="button" class="btn btn-sm btn-danger" data-toggle='tooltip' data-placement='left' title='Rencanakan {{$perina->namapasien}} pulang' onclick="return  confirm('Apakah anda yakin?')"><i class="fas fa-user-injured"></i> <b>RENCANA PULANG</b></a>
                    
                      @elseif ($perina->bedstatus==2)
                      <a href="{{ route('perinatologi.batalrencanaPulang', $perina->trxPasien->trx_id) }}" type="button" class="btn btn-sm btn-warning" data-toggle='tooltip' data-placement='left' title='Batalkan {{$perina->namapasien}} rencana pulang' onclick="return  confirm('Apakah anda yakin?')"><i class="fas fa-procedures"></i> <b>BATAL RENCANA PULANG</b></a>
                      <a href="{{ route('perinatologi.pulangkanPasien', $perina->trxPasien->trx_id) }}" type="button" class="btn btn-sm btn-danger" data-toggle='tooltip' data-placement='left' title='Pulangkan {{$perina->namapasien}}' onclick="return  confirm('Apakah anda yakin akan memulangkan pasien tersebut? tidak bisa dibatalkan apabila telah dipulangkan.')"><i class="fas fa-sign-out-alt"></i> <b>PULANGKAN PASIEN</b></a>
                    @endif
                  </div>
                </div>    
                {{-- modal --}}
                <div class="modal fade" id="Modal-{{$perina->trx_id}}" tabindex="-1" aria-labelledby="modal" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered" role="document">
                      <div class="modal-content">
                          ...
                          <form action="{{ route('perinatologi.updatebed', $perina->trx_id) }}" method="post">
                            @csrf
