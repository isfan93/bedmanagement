  ...
  <div class="card-body">
    ...
    <tbody>
      @foreach ($pxBookings as $pxBooking)
        @if ($pxBooking->bedTujuan->ruangan == 'AKASIA')
          <tr>
            <td>
              ...
              <td  width='89px' style="text-align: center"> 
                <div class="btn-group">
                  @if($pxBooking->bedTujuan->bedstatus<>3)
                    <a href="{{ route('akasia.approveBooking', $pxBooking->trxPasien->trx_id) }}" class='btn btn-sm btn-primary' data-toggle='tooltip' data-placement='left' title='Approve pasien' onclick="return  confirm('Apakah anda yakin?')"><i class="fas fa-check"></i></a>
                  @endif
                  <a href="{{ route('akasia.batalranap', $pxBooking->trxPasien->trx_id) }}" class='btn btn-sm btn-danger' data-toggle='tooltip' data-placement='left' title='Cancel pasien' onclick="return  confirm('Apakah anda yakin?')"><i class="fas fa-times-circle"></i></a>
                </div>
              </td>
            @endif
          </tr>
